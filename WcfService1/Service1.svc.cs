...
namespace WcfService1
{
    ...
    public class Service1 : IFcsuServices
    {
...
//                using (var context = new FcsuAgentEntities())
                {
                    var context = new FcsuAgentEntities();                    
        ...
        public IsUserValidResponse IsUserValid(FcsuAgent.Services.IsUserValidRequest request)
        {
            try
            {
...
//                        response.MemberNumber = f.MemberNumber;
        ...
        public GetUserListResponse GetUserList(GetUserListRequest request)
        {
            try
            {
                ...
                using (var context = new FcsuAgentEntities())
                {
                    ...
                    response.UserList = query.OrderBy(x => x.LastName).ToList(); //order by lastname  09/28/2020 for members,agent,director,no role assigned lists
                }
        ...
        public DeleteUserResponse DeleteUserResponse(DeleteUserRequest request)
        {
            try
            {
                ...
                using (var context = new FcsuAgentEntities())
                {
                    ...
                    if (lookupUser != null)
                    {
                        ...
                        //wipe out all existing x-links for role
                        foreach (var userInRole in context.UserAndRoleXLinks.Where(x => x.UserId == request.UserPk))
                        {
                            context.UserAndRoleXLinks.DeleteObject(userInRole);
                        }
                        context.SaveChanges();
                        ...
                        context.SaveChanges();
                        response.FailureMessage = "Successfully deleted";
                        response.Success = true;
                    }
                    else
                    {
            ...
            {
                LoggingMediator.Log(eX, LogEventType.Exception);
                return new DeleteUserResponse { Success = false, FailureMessage = "Unable to delete the User::" };
            }
        }
        //Purnima
    }
